...
def create_mock_chat_messages(category: str, confidence: float = 0.9) -> list[dict]:
    ...
    return [
        ...
        {"name": "DiagnosisAgent", "content": f"```json\n{json.dumps(mock_diagnosis)}\n```"}
    ]

@pytest.mark.asyncio
@pytest.mark.parametrize("category, expected_enum", [
...
async def test_diagnosis_categories(mock_create_agents, mock_create_chat, category, expected_enum):
    ...
    mock_create_agents.return_value = (mock_coordinator, MagicMock(), MagicMock(), MagicMock())
