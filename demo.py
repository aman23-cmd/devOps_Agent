...
def trigger_fake_failure():
    ...
    error_types = ["Code Regression (pytest failed)", "Dependency Issue (npm ERR!)", "Flaky Test (timeout)"]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("You should see the new failure pop up and the agent will start fixing it!")
        else:
            print(f"❌ Failed. Status Code: {response.status_code}")
            print("Response:", response.text)
    except requests.exceptions.ConnectionError:
        print("❌ ERROR: Could not connect to the webhook receiver.")
        print("Make sure your FastAPI server is running: python -m uvicorn api.webhook_receiver:app --reload")
